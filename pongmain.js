...
let rangepos = document.querySelector("#range")
...
if (window.matchMedia("(max-width: 767px").matches) {
    canvas.width = Math.floor(window.innerWidth) - (window.innerWidth * 0.01);
    canvas.height = Math.floor(window.innerWidth) - (window.innerWidth * 0.2);
}
else{
    canvas.width = Math.floor(window.innerWidth) - (window.innerWidth * 0.5);
    canvas.height = Math.floor(window.innerWidth) - (window.innerWidth * 0.6);
}
let fps = 30
let mode = 0;
let running = false
...
function isgoing(){
    ...
    if (easy.checked + medium.checked + impossible.checked > 1) {
